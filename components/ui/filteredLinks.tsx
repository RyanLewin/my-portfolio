...
function FilteredLink({ link, activeFilters }: { link: CustomLink, activeFilters: string[] }) {
    return (
        <Link key={link.id} href={link.href} className="max-w-full sm:max-w-[45%] lg:max-w-[30%] flex flex-col flex-wrap justify-start gap-2">
            <Image
                ...
                width={320}
            />
            <p className="text-center text-2xl font-bold">{link.title}</p>
            <p className="w-full text-justify mb-4 overflow-hidden line-clamp-4">{link.contribution}</p>
            ...
            <p className="text-center">{link.timeline}</p>
        </Link>
    )
}
